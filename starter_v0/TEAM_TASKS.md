# AI Trend Detective — Team Task Plan (3 people)
...
- `Member A` should not change team eval cases yet.
...
1. Member A reports baseline status and `v0` metrics.
2. Member B confirms UI works and shows the trace.
3. Member C shares tool implementation and eval cases.
4. Agree on any prompt/tool wording changes together.

### File ownership summary

- `Member A`: `artifacts/system_prompt.md`, baseline `v0`, preflight checks, `artifacts/version_log.csv`.
- `Member B`: `app.py`, UI trace display, `requirements.txt`, demo writeup in `artifacts/REPORT.md`
- `Member C`: `tools/evidence_judge`, `tools/__init__.py`, `artifacts/tools.yaml`, `data/eval_group.json`, `run_eval` for team eval.
...
- Member A: `python run_eval.py --provider openrouter --version v0 --suite base --eval-cases data/eval_base.json`
- Member B: `streamlit run app.py` works and UI shows tool trace.
- Member C: `python run_eval.py --provider openrouter --version v2 --suite group --eval-cases data/eval_group.json`
...
1. Assign actual names to Member A, B, C.
